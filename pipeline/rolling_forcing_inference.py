...
# Adapted from Rolling-Forcing under its academic-use license.
# See third_party/rolling_forcing/LICENSE and THIRD_PARTY_NOTICES.md.
...
class RollingForcingInferencePipeline(torch.nn.Module):
    def __init__(
            self,
            args,
            device,
        ...
        self.num_attention_heads = self.generator.model.num_heads
        self.attention_head_dim = self.generator.model.dim // self.num_attention_heads
    ...
    def inference(
        ...
        if self.kv_cache_clean is None:
            ...
            for block_index in range(len(self.kv_cache_clean)):
                reset_kv_cache(self.kv_cache_clean[block_index])
        ...
        for window_index in tqdm(range(window_num), desc="Rolling windows"):
    ...
    def _initialize_kv_cache(self, batch_size, dtype, device):
        ...
        for _ in range(self.num_transformer_blocks):
            kv_cache_clean.append({
                "k": torch.zeros(
                    [batch_size, kv_cache_size, self.num_attention_heads, self.attention_head_dim],
                    dtype=dtype,
                    device=device,
                ),
                "v": torch.zeros(
